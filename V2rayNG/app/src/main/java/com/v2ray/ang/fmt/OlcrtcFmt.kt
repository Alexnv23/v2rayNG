...
/**
 * SuperNet: парсер/сериализатор ссылок запасного канала olcRTC.
 * Формат: olcrtc://<carrier>?<transport>[<k=v&...>]@<room>#<keyHex>[$<remarks>]
 * Пример: olcrtc://jitsi?datachannel@myroom#<64hex>$Запасной%20канал
 * (портировано из venterum/veil)
 */
object OlcrtcFmt : FmtBase() {
    ...
    fun toUri(config: ProfileItem): String {
        ...
        buildEngineWithClient(config)?.let {
            sb.append("<")
            sb.append(it)
            sb.append(">")
        }
